...
export default async function AdminChannelRequestsPage(
  ...
  return (
    <div>
      ...
      <p className="muted">
        Requests for a new Town Hall channel. Approving creates the channel immediately and
        messages the requester to let them know.
      </p>
        ...
        {pending.map((r: any) => (
          <div key={r.id} style={{ borderBottom: "1px solid var(--border)", paddingBottom: "0.75rem", marginBottom: "0.75rem" }}>
            ...
            <div style={{ marginTop: "0.5rem" }}>
              <form action={reviewChannelRequest} style={{ display: "inline" }}>
      ...
      <div className="card">
        ...
        {reviewed.map((r: any) => (
          <p key={r.id}>
            {r.channel_name} <span className={`tag ${r.status === "approved" ? "" : "muted"}`}>{r.status}</span>
          </p>
        ))}
