...
function TierBadge({ tier }: { tier: "partner" | "trusted_colleague" | "bench" | "pending-out" | "pending-in" | null }) {
  ...
  if (tier === "bench") return <span className="tag">Saved clinician</span>;
...
export default async function PersonProfilePage({
  ...
  // Professional matching uses member-selected specialisms and state only.
  const highlightValues = (myLookups || []).map((l: any) => l.lookup_values?.value).filter(Boolean) as string[];
  const locationMatch = !!myProfile?.primary_state && myProfile.primary_state === first.primary_state;
  ...
  const MatchToggle = () => <span className="tag">Shared professional interests</span>;
