...
import { rankCandidates, rankCandidatesByGrid, type ConnectionTier, type MatchCandidate, type GridCandidate } from "@/lib/matching";
...
// Grid-based ranking (see rankCandidatesByGrid in @/lib/matching) for the
// newer features: Caseload's Quick Match, Planner's per-client recommendation
// table, and the Single Patient Referral tool. Deliberately separate from
// computeRankedCandidates above so the existing coverage-plan flow's
// candidate ordering is untouched.
export async function computeGridRankedCandidates(
  supabase: SupabaseClient,
  requesterId: string,
  need: {
    specialismValue: string | null;
    state?: string | null;
    sessionType?: string | null;
    insuranceValue?: string | null; // set only when the caller wants insurance-matched candidates
  },
  excludeProfileIds: string[] = []
): Promise<
  Array<
    ReturnType<typeof rankCandidatesByGrid>[number] & {
      city: string | null;
      state: string | null;
      acceptingReferrals: boolean;
      acceptsInsurance: boolean | null; // null = insurance wasn't checked for this call
    }
  >
> {
  const [{ data: directoryRows }, { data: connections }, { data: scores }, { data: blocklist }, insurancePanelsQuery] =
    await Promise.all([
      supabase.from("public_directory").select("*"),
      supabase
        .from("connections")
      ...
      supabase.from("do_not_work_with").select("blocked_profile_id").eq("profile_id", requesterId),
      need.insuranceValue
        ? supabase.from("insurance_panels").select("profile_id, insurance_name")
  ...
  const insuranceLower = need.insuranceValue?.trim().toLowerCase() || null;
  const acceptsInsuranceById = new Set<string>();
  if (insuranceLower) {
    for (const row of (insurancePanelsQuery as any).data || []) {
      if (String(row.insurance_name || "").trim().toLowerCase() === insuranceLower) {
        acceptsInsuranceById.add(row.profile_id);
      }
    }
  }
  ...
  const candidates: GridCandidate[] = Array.from(people.values())
    .filter((p) => p.accepting_referrals)
    .filter((p) => !need.specialismValue || p.specialismRankByValue.has(need.specialismValue))
    .map((p) => ({
      profileId: p.id,
      fullName: `${p.credential_prefix || ""} ${p.full_name}`.trim(),
      connectionTier: tierByOtherId.get(p.id) ?? "none",
      specialtyRank: need.specialismValue ? p.specialismRankByValue.get(need.specialismValue) ?? null : null,
      endorsementScore: scoreById.get(p.id) || 0,
      lastActiveAt: p.last_active_at,
      qualifiedForState: !need.state || p.primary_state === need.state || p.psypact_participating,
      sessionTypeMatch: !need.sessionType || p.sessionTypes.size === 0 || p.sessionTypes.has(need.sessionType),
    }));

  const ranked = rankCandidatesByGrid(candidates);
  const infoById = new Map(Array.from(people.values()).map((p) => [p.id, p]));
  return ranked.map((c) => {
    const info = infoById.get(c.profileId);
    return {
      ...c,
      city: info?.primary_practice_city ?? null,
      state: info?.primary_state ?? null,
      acceptingReferrals: info?.accepting_referrals ?? true,
      acceptsInsurance: insuranceLower ? acceptsInsuranceById.has(c.profileId) : null,
    };
  });
}
